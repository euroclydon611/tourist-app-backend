...
exports.destinationReview = exports.getDestinationById = exports.fetchDestinations = exports.updateDestination = exports.createDestination = void 0;
 ...
 */
exports.createDestination = (0, catchAsyncErrors_1.default)(async (req, res, next) => {
    const { name, region, description, shortDescription, rating, coordinates, topAttractions, tags, } = req.body;
    ...
    try {
        await destinations_model_1.default.create({
            name,
            region,
            description,
            shortDescription,
 ...
 * @access  Protected/Admin
 */
exports.updateDestination = (0, catchAsyncErrors_1.default)(async (req, res, next) => {
    const destinationId = req.params.id;
    const { name, region, description, shortDescription, rating, coordinates, topAttractions, tags, } = req.body;
    try {
        const destination = await destinations_model_1.default.findById(destinationId);
        if (!destination) {
...
exports.fetchDestinations = (0, catchAsyncErrors_1.default)(async (req, res, next) => {
    ...
    const geoOptions = lat && lng
        ...
        : [{ $match: query }];
    try {
        const destinations = await destinations_model_1.default.aggregate([
            ...geoOptions,
            { $skip: skip },
            { $limit: limitNum },
        ]);
        const total = await destinations_model_1.default.countDocuments(query);
        res.status(200).json({
            success: true,
            data: destinations,
            pagination: {
...
exports.getDestinationById = (0, catchAsyncErrors_1.default)(async (req, res, next) => {
    const { id } = req.params;
    const destination = await destinations_model_1.default.findById(id);
    if (!destination) {
        return next(new ErrorHandler_1.default("Destination not found", 404));
    }
    res.status(200).json({
...
exports.destinationReview = (0, catchAsyncErrors_1.default)(async (req, res, next) => {
    var _a;
    try {
